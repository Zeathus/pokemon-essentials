...
class PokemonPokedex_Scene
  ...
  def pbStartScene
    ...
    @typebitmap2        = AnimatedBitmap.new(_INTL("Graphics/Pictures/types"))
...
=end
  ...
  def pbRefresh
    ...
    if iconspecies
      ...
      textpos.push([species_data.name,240,46,2,base,shadow])
      type1 = GameData::Type.get(species_data.type1).id_number
      type2 = GameData::Type.get(species_data.type2).id_number
      affinity = GameData::Type.get(species_data.affinity1).id_number
      if type1 == type2
    ...
    if @searchResults
      textpos.push([_INTL("Search results"),236,454,2,base,shadow])
      textpos.push([@dexlist.length.to_s,236,484,2,base,shadow])
    else
      textpos.push([_INTL("Seen:"),162,454,0,base,shadow])
      textpos.push([$Trainer.pokedex.seen_count(pbGetPokedexRegion).to_s,302,454,1,base,shadow])
      textpos.push([_INTL("Owned:"),162,484,0,base,shadow])
      textpos.push([$Trainer.pokedex.owned_count(pbGetPokedexRegion).to_s,302,484,1,base,shadow])
    end
    # Draw all text
