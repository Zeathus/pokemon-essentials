...
class PokemonBoxPartySprite < SpriteWrapper
  def initialize(party,viewport=nil)
    ...
    @pokemonsprites = []
    for i in 0...Settings::MAX_PARTY_BOX_SIZE
      @pokemonsprites[i] = nil
      if i < Settings::MAX_PARTY_SIZE
        ...
        pokemon = $Trainer.inactive_party[i-Settings::MAX_PARTY_SIZE]
        if pokemon
          @pokemonsprites[i] = PokemonBoxIcon.new(pokemon,viewport)
        end
      end
  ...
  def compact
    ...
    p2 = @pokemonsprites[Settings::MAX_PARTY_SIZE...Settings::MAX_PARTY_BOX_SIZE].compact
    @pokemonsprites = p1
  ...
  def refresh
    ...
    xvalues = []   # [18, 18, 18, 90, 90, 90]
    yvalues = []   # [2, 66, 130, 18, 82, 146]
    for i in 0...Settings::MAX_PARTY_BOX_SIZE
...
class PokemonStorageScene
  ...
  def pbPartySetArrow(arrow,selection)
    ...
    elsif selection < Settings::MAX_PARTY_BOX_SIZE
  ...
  def pbPartyChangeSelection(key,selection)
    ...
    when Input::LEFT
      selection -= 3
      selection = Settings::MAX_PARTY_BOX_SIZE if selection < 0
    when Input::RIGHT
      selection += 3
      selection = 0 if selection > Settings::MAX_PARTY_BOX_SIZE
    when Input::UP
      if selection == Settings::MAX_PARTY_BOX_SIZE
        selection = Settings::MAX_PARTY_BOX_SIZE - 1
      else
        selection -= 1
        selection = Settings::MAX_PARTY_BOX_SIZE if selection < 0
      end
    when Input::DOWN
      if selection == Settings::MAX_PARTY_BOX_SIZE
        ...
        selection += 1
        selection = Settings::MAX_PARTY_BOX_SIZE if selection > Settings::MAX_PARTY_BOX_SIZE
      end
    end
    return selection
  end
    ...
    loop do
      ...
      if Input.trigger?(Input::ACTION) && @command == 0   # Organize only
        ...
        if selection >= 0 && selection < Settings::MAX_PARTY_SIZE
          @selection = selection
          return selection
        elsif selection >= Settings::MAX_PARTY_SIZE && selection < Settings::MAX_PARTY_BOX_SIZE
          # Add to off-party
          @selection = selection
          return selection
        elsif selection == Settings::MAX_PARTY_BOX_SIZE   # Close Box
          @selection = selection
  ...
  def pbUpdateOverlay(selection,party=nil)
    ...
    if @screen.pbHeldPokemon
      ...
      pokemon = (party) ? (selection < Settings::MAX_PARTY_SIZE ? party[selection] : $Trainer.inactive_party[selection-3]) :
                @storage[@storage.currentBox,selection]
    end
    if !pokemon
...
class PokemonStorageScreen
      ...
      loop do
        ...
        if selected==-3   # Close box
          ...
          if selected >= Settings::MAX_PARTY_SIZE
            pokemon = $Trainer.inactive_party[selected-3]
          end
          next if !pokemon
          command = pbShowCommands(_INTL("{1} is selected.",pokemon.name),[
             _INTL("Store"),
  ...
  def pbWithdraw(selected,heldpoke)
    ...
    if @storage.party.length < Settings::MAX_PARTY_SIZE
      ...
      @scene.pbWithdraw(selected,heldpoke,$Trainer.inactive_party.length+3)
    end
    if heldpoke
  ...
  def pbStore(selected,heldpoke)
    ...
    if pbAbleCount<=1 && !(box==-1 && index >= Settings::MAX_PARTY_SIZE)
       pbAble?(@storage[box,index]) && !heldpoke
      pbPlayBuzzerSE
  ...
  def pbHold(selected)
    box = selected[0]
    index = selected[1]
    if box==-1 && !(box==-1 && index >= Settings::MAX_PARTY_SIZE) &&
       pbAble?(@storage[box,index]) && pbAbleCount<=1
      pbPlayBuzzerSE
      pbDisplay(_INTL("That's your last Pokémon!"))
      return
    end
