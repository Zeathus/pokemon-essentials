...
class PokeSelectionSprite < SpriteWrapper
  ...
  def refresh
    ...
    if @itemsprite && !@itemsprite.disposed?
      @itemsprite.visible=(@pokemon.item && (@pokemon.item>0))
        ...
        loop do
          ...
          # Teach move
            ...
            if Input.press?(Input::CTRL)
              Kernel.pbMessage("Please enter the Move name.")
              poke=PokeBattle_Pokemon.new(:UNOWN,10,$Trainer)
              movename=pbEnterText("Name:",0,20,"",2,poke)
              moveid = getID(PBMoves,movename.upcase)
              moveid = -1 if !moveid.is_a?(Integer)
              if moveid < 1
                Kernel.pbMessage("Invalid Move name.")
              else
                pbLearnMove(pkmn,moveid)
                pbRefreshSingle(pkmnid)
              end
            else
