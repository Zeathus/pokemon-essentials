class ShellOSScreen
    ...
    def pbSetupSequence
        ...
        pbFadeInAndShow(@sprites)
        pbWait(60)
        pbMessage(_INTL("{1} grabs their Trainer Card and scans it.", $Trainer.name))
        temp_text = all_text + "\nScanning Trainer Card..."
        pbLoading(terminal, temp_text, 4)
        all_text += "\nUser identified as '" + $Trainer.name + "'."
        terminal.text = all_text
        pbWait(60)
        temp_text = all_text + "\nSearching in user database..."
        pbLoading(terminal, temp_text, 6)
        all_text += "\nCould not find '" + $Trainer.name + "'."
        terminal.text = all_text
        pbWait(60)
        all_text += "\nCreate new user '" + $Trainer.name + "'? (y/n): "
        terminal.text = all_text
        pbWait(60)
        all_text += "y"
        terminal.text = all_text
        ...
        temp_text = all_text + "\nCreating new user..."
        pbLoading(terminal, temp_text, 4)
        all_text += "\nUser '" + $Trainer.name + "' successfully created."
        terminal.text = all_text
        pbWait(60)
