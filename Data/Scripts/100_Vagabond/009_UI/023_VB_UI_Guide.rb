...
class KeybindSprite < SpriteWrapper
  ...
  def refresh
    ...
    if @input.is_a?(Array)
      ...
      textpos = [[@name,x+6,0,0,@baseColor,@shadowColor,true]]
      pbSetSmallFont(self.bitmap)
      pbDrawTextPositions(self.bitmap, textpos)
    else
      self.bitmap.blt(0, 0, @keybind_icons.bitmap, $Keybinds.rect(@input))
      textpos = [[@name,34,0,0,@baseColor,@shadowColor,true]]
      pbSetSmallFont(self.bitmap)
      pbDrawTextPositions(self.bitmap, textpos)
    end
  end
...
class GuideScreen
  ...
  def pbStartGuideScreen(start_guide=nil)
    ...
    if start_guide
      ...
      for i in 0...@guides.length
        if @guides[i][0] == $game_variables[LAST_GUIDE]
          @index = i
          break
        end
      end
    ...
    @sprites["control_exit"] = KeybindSprite.new(Input::BACK, "Exit", 180, Graphics.height - 36, @viewport)
    @sprites["control_hint"] = KeybindSprite.new([Input::UP, Input::DOWN], "Select Hint", 264, Graphics.height - 36, @viewport)
    @sprites["control_page"] = KeybindSprite.new([Input::LEFT, Input::RIGHT], "Change Page", 436, Graphics.height - 36, @viewport)
    self.refresh
  ...
  def refresh
    ...
    $game_variables[LAST_GUIDE] = item[0]
    @sprites["overlay"].bitmap.clear
    page_text = _INTL("Page {1}/{2}", @page+1, (item == 0) ? 1 : item.length-1)
    textpos = [
      [page_text, Graphics.width-98, Graphics.height-78, 0, Color.new(252,252,252), Color.new(0,0,0), true]
    ]
...
def pbInitGuides
  ...
  guides["Types"] = true
  guides["Stats"] = true
  $game_variables[GUIDE_LIST] = guides
end

def pbHasGuide(guide)
  pbInitGuides if $game_variables[GUIDE_LIST].is_a?(Numeric)
  return $game_variables[GUIDE_LIST][guide]
end

def pbAddGuide(guide)
  pbInitGuides if $game_variables[GUIDE_LIST].is_a?(Numeric)
  if !$game_variables[GUIDE_LIST][guide]
    $game_variables[GUIDE_LIST][guide] = true
    $game_variables[NEW_GUIDE] = guide
  end
end
