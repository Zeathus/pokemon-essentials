def compile_quests(path = "PBS/quests.txt")
  ...
  File.open(path, "rb") { |f|
    ...
    pbEachFileSection(f) { |contents, quest_number|
      FileLineData.setSection(quest_number, "header", nil)   # For error reporting
      # Raise an error if a quest number is invalid or used twice
      if quest_number == 0
        raise _INTL("A quest can't be numbered 0 ({1}).", path)
      elsif GameData::Quest::DATA[quest_number]
        raise _INTL("Quest ID number '{1}' is used twice.\r\n{2}", quest_number, FileLineData.linereport)
      end
      contents["Steps"] = []
      contents["MapGuides"] = []
      ...
      for key in schema.keys
        ...
        if nil_or_empty?(contents[key])
          if ["Name", "InternalName"].include?(key)
            raise _INTL("The entry {1} is required in {2} section {3}.", key, path, quest_number)
          end
          contents[key] = nil
          next
        end
        # Raise an error if a quest internal name is used twice
        FileLineData.setSection(quest_number, key, contents[key])   # For error reporting
      ...
      quest_names[quest_number] = quest_hash[:name]
    }
  }
  # Save all data
  GameData::Quest.save
