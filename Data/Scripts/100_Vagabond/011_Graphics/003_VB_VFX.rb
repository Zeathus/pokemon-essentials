...
def pbUpdateVFX
  leaves = [PBMaps::BrecciaUndergrowth]
  leaves_autumn = [PBMaps::SouthGabbroRiver,
                   PBMaps::AmphiTown,
                   PBMaps::RangerOutpost,
                   233] # Ranger Outpost Cutscene
  if leaves.include?($game_map.map_id)
    $game_screen.vfx(:Leaves) if $game_screen.vfx_type != PBVFX::Leaves
  elsif leaves_autumn.include?($game_map.map_id)
    $game_screen.vfx(:LeavesAutumn) if $game_screen.vfx_type != PBVFX::LeavesAutumn
  else
    $game_screen.vfx(:None) if $game_screen.vfx_type != PBVFX::None
  end
end
