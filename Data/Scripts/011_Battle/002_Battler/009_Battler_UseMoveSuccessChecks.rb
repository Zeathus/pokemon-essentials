class Battle::Battler
  ...
  def pbCanChooseMove?(move,commandPhase,showMessages=true,specialUsage=false)
    ...
    # Imprison
<<<<<<< HEAD:Data/Scripts/011_Battle/001_Battler/009_Battler_UseMove_SuccessChecks.rb
...
=======
    if @battle.allOtherSideBattlers(@index).any? { |b| b.effects[PBEffects::Imprison] && b.pbHasMove?(move.id) }
      if showMessages
        msg = _INTL("{1} can't use its sealed {2}!",pbThis,move.name)
        (commandPhase) ? @battle.pbDisplayPaused(msg) : @battle.pbDisplay(msg)
>>>>>>> 479aeacc2c9dddad1b701c1a92a2a1f915e34388:Data/Scripts/011_Battle/002_Battler/009_Battler_UseMoveSuccessChecks.rb
      end
    end
